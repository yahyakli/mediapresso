                            ...
                            <>
                                <div className="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
                                    {post.attachments.map((_, index) => (
                                        <button key={index} type="button" className={`w-3 h-3 rounded-full ${index === 0 ? 'bg-blue-600' : 'bg-white'}`} aria-current={index === 0 ? "true" : "false"} aria-label={`Slide ${index + 1}`} data-carousel-slide-to={index}></button>
                                    ))}
                                </div>
                                <button type="button" className="absolute top-1/2 left-0 z-30 flex items-center justify-center px-4 cursor-pointer group focus:outline-none -translate-y-1/2" data-carousel-prev>
                                    <span className="text-white dark:text-gray-400 dark:hover:text-white transition-all inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-100 group-hover:bg-white/50 dark:group-hover:bg-gray-300 dark:group-focus:ring-gray-800/70 group-focus:outline-none">
